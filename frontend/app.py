...
def display_preliminary_data(prelim: Dict):
    ...
    # Handle case where prelim might have status message instead of pilot_studies
    if prelim.get('status'):
        st.info(prelim.get('status'))
        return
    
    pilots = prelim.get("pilot_studies", [])
    # Ensure pilots is a list, not string
    if not isinstance(pilots, list):
        st.info("No preliminary studies conducted yet. This hypothesis requires validation.")
        return
    
    for pilot in pilots[:3]:
        # Ensure each pilot is a dict

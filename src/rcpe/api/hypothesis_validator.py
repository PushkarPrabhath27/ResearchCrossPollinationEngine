...
class HypothesisValidator:
    ...
    # BANNED CROSS-DOMAIN PHRASES (from ummnew.md - too vague)
    BANNED_CROSS_DOMAIN_PHRASES = [
    ...
    # BANNED METHODOLOGY JUSTIFICATIONS (from ummnew.md)
    BANNED_METHODOLOGY_PHRASES = [
        "standard practice",
        "commonly used",
        "typical value",
    ...
    def build_paper_lookup(self, retrieved_papers: List[Dict]) -> None:
        ...
        IMPROVED: Includes ALL author last names, not just first author.
        """
        ...
        for paper in retrieved_papers:
            ...
            # Author-year lookup - include ALL authors
            ...
            if authors and authors != "Unknown" and year:
                ...
                for author_full in author_parts:
                    author_full = author_full.strip()
                    if not author_full:
                        continue
                    
                    # Get last name (last word that's not an initial)
                    parts = author_full.split()
                    for i in range(len(parts) - 1, -1, -1):
                        part = parts[i].strip()
                        # Skip initials like "R." or suffixes
                        if len(part) > 2 and not part.endswith('.'):
                            last_name = part.lower()
                            # Add all citation patterns
                            self.retrieved_authors_cache.add(f"{last_name} ({year})")
                            self.retrieved_authors_cache.add(f"{last_name} et al. ({year})")
                            self.retrieved_authors_cache.add(f"{last_name} {year}")
                            self.retrieved_authors_cache.add(f"{last_name}, {year}")
                            self.retrieved_authors_cache.add(last_name)  # Just the name
                            break
    ...
    def extract_citations_from_text(self, text: str) -> List[str]:
        ...
        IMPROVED: Filters out false positives like 'Enables (1000)' or 'Available (2024)'
        """
        citations = []
        
        # Words that look like author names but aren't
        FALSE_POSITIVES = {
            'enables', 'available', 'published', 'accessed', 'retrieved',
            'updated', 'created', 'modified', 'version', 'release',
            'figure', 'table', 'section', 'chapter', 'page', 'equation',
            'method', 'results', 'discussion', 'conclusion', 'abstract',
            'source', 'format', 'size', 'output', 'input', 'step',
            'week', 'month', 'year', 'day', 'hour', 'minute', 'second'
        }
        
        # Pattern: Author et al. (Year) - must be realistic year (1900-2030)
        pattern1 = r'([A-Z][a-z]+(?:\s+et\s+al\.)?)\s*\((\d{4})\)'
        matches = re.findall(pattern1, text)
        for author, year in matches:
            author_clean = author.strip()
            year_int = int(year)
            # Filter: valid year range and not a false positive word
            if 1900 <= year_int <= 2030 and author_clean.lower().split()[0] not in FALSE_POSITIVES:
                citations.append(f"{author_clean} ({year})")
    ...
    def check_banned_phrases(self, text: str) -> Tuple[int, List[str]]:
        """
        Check for banned phrases from ummnew.md that indicate low-quality output.
        Returns (count, list of found phrases with context)
        """
        text_lower = text.lower()
        found_phrases = []
        
        # Check cross-domain banned phrases
        for phrase in self.BANNED_CROSS_DOMAIN_PHRASES:
            if phrase.lower() in text_lower:
                ...
                found_phrases.append(f"BANNED CROSS-DOMAIN: '{phrase}' in '...{context}...'")
        
        # Check methodology banned phrases  
        for phrase in self.BANNED_METHODOLOGY_PHRASES:
            if phrase.lower() in text_lower:
                idx = text_lower.find(phrase.lower())
                start = max(0, idx - 20)
                end = min(len(text), idx + len(phrase) + 20)
                context = text[start:end].replace('\n', ' ').strip()
                found_phrases.append(f"BANNED METHODOLOGY: '{phrase}' in '...{context}...'")
    ...
    def validate(
        ...
        # 4b. Banned PHRASES check (NEW from ummnew.md)
        phrase_count, banned_phrases = self.check_banned_phrases(hypothesis_text)
        if phrase_count > 0:
            # Heavy penalty for banned phrases
            specificity = max(0, specificity - phrase_count * 1.0)
            issues.append(f"QUALITY ISSUE: {phrase_count} banned phrases found (e.g., 'bio-inspired design principles')")
            issues.extend(banned_phrases[:2])
